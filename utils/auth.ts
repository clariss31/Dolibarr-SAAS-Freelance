...
export const auth = {
  ...
  setAuth: (token: string, apiUrl: string) => {
    // Les cookies expirent après 7 jours par défaut, ou selon vos besoins
    Cookies.set(TOKEN_KEY, token, { expires: 7, secure: true, sameSite: 'lax' });
    Cookies.set(API_URL_KEY, apiUrl, { expires: 7, secure: true, sameSite: 'lax' });
  },
  
  clearAuth: () => {
    Cookies.remove(TOKEN_KEY);
    Cookies.remove(API_URL_KEY);
    // On nettoie aussi le localStorage par précaution (ancienne version)
